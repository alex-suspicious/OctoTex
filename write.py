    ...
    for x in tqdm(os.listdir("textures/processing/diffuse/"), desc="Converting..."):
        if x.endswith(".png"):
            try:
                ...
                if (not already):
                    with image.Image(filename=f"textures/processing/upscaled/{x}") as img:
                        ...
                        img.save(
                            filename=f"{config.rtx_remix_dir}/mods/{mod_dir}/SubUSDs/textures/diffuse/{x.replace('png', 'dds')}")
            ...
            try:
                ...
                if (not already):
                    with image.Image(filename=f"textures/processing/normals/{x.replace('.png', '')}_normal.png") as img:
                        img.compression = "dxt5"
                        img.save(
                            filename=f"{config.rtx_remix_dir}/mods/{mod_dir}/SubUSDs/textures/normals/{x.replace('png', 'dds')}")
            ...
            try:
                ...
                if (not already):
                    with image.Image(
                        ...
                        img.compression = "dxt5"
                        img.save(
                            filename=f"{config.rtx_remix_dir}/mods/{mod_dir}/SubUSDs/textures/roughness/{x.replace('png', 'dds')}")
            ...
            try:
                ...
                if (not already):
                    with image.Image(
                            filename=f"textures/processing/displacements/{x.replace('.png', '')}_disp.png") as img:
                        img.compression = "dxt5"
                        img.save(
                            filename=f"{config.rtx_remix_dir}/mods/{mod_dir}/SubUSDs/textures/displacements/{x.replace('png', 'dds')}")
...
emissive_intensity = 0"""
            ...
            mat_to_edit["over Shader"][
                "asset inputs:diffuse_texture"] = f"@./SubUSDs/textures/diffuse/{mat_selected_name}.dds@"
            mat_to_edit["over Shader"][
